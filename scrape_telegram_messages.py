...
for chat in chats:
    with TelegramClient('messages', api_id, api_hash) as client:
        for message in client.iter_messages(chat, offset_date=datetime.date.today() - datetime.timedelta(days=2), reverse=True): # right now it's set to get chat history from 2 days ago to current date
            print(message)
